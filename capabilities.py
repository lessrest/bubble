...
class Capability(ABC):
    ...
    @abstractmethod
    async def execute(self, parameter: URIRef, invocation: URIRef, graph: Graph) -> None:
...
class ShellCapability(Capability):
    ...
    async def execute(self, parameter: str, invocation: URIRef, graph: Graph) -> None:
        ...
        try:
            from n3 import FileHandler, SWA
...
class ArtGenerationCapability(Capability):
    ...
    async def execute(self, parameter: URIRef, invocation: URIRef, graph: Graph) -> None:
        from n3 import FileHandler, SWA, NT
