...
async def receive_results(client: WebSocketConnection, results: URIRef):
    while True:
        ...
        if message.channel.alternatives[0].transcript:
            ...
            try:
                rml_output, _ = message.process_rml()
                with with_transient_graph() as id:
                    new(
                        Deepgram.Result,
                        {
                            ...
                            Deepgram.rml_output: rml_output,
                        },
                        id,
                    )
                    await send(results)
            except Exception as e:
                logger.error("Error processing RML mapping", error=str(e))
                # Continue with original behavior if RML fails
                with with_transient_graph() as id:
                    new(
                        Deepgram.Result,
                        {
