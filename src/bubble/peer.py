...
class Peer:
            ...
            async with trio_websocket.open_websocket_url(ws_url) as ws:
                ...
                for subject in handshake.subjects(None, NT.Handshake):
                    ...
                    if signed_question:
                        ...
                        response = Graph()
                        response.add((subject, NT.signedAnswer, 
                                    URIRef(base64.b64encode(signed_answer).decode())))
                        
                        # Send response
                        await ws.send_message(response.serialize(format="json-ld"))
                ...
                trio_websocket.ConnectionClosed) as e:
            logger.error("websocket error", error=e)
            raise

    async def _receive_messages(self, ws: trio_websocket.WebSocketConnection) -> None:
    ...
    async def _heartbeat(self, ws: trio_websocket.WebSocketConnection) -> None:
